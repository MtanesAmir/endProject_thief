...
class ScentTracker:
    ...
    def update_scent(self, center_pos: Tuple[int, int], tau_center: float = DEFAULT_TAU_CENTER, rho: float = DEFAULT_RHO) -> None:
        retention = 1.0 - rho
        
        for r in range(self.grid_size):
            for c in range(self.grid_size):
                self.matrix[r][c] *= retention
        ...
        for r in range(self.grid_size):
            for c in range(self.grid_size):
                self.matrix[r][c] = max(0.0, self.matrix[r][c])
